#!/bin/bash
# Adds Jest tests for the analysis rules. Does not change any threshold:
# the merge gap stays at the agreed 120 seconds.
# Run from the repository root:  bash add-tests.sh

set -e

if [ ! -f "server/src/analysis.js" ]; then
  echo "server/src/analysis.js not found. Run this from the MScProject folder."
  exit 1
fi

mkdir -p server/tests

cat > server/tests/analysis.test.js << 'EOF'
...
echo "Tests written. Run: cd server && npm test"
