#!/bin/bash
# Fixes the API tests: jest.mock is hoisted above const declarations, so the
# fixtures move into their own module and the stub loads them when called.
# Run from the repository root:  bash fix-api-tests.sh

set -e

if [ ! -d "server/tests" ]; then
  echo "No server/tests folder here. Run this from the MScProject folder."
  exit 1
fi

# ---- fixtures --------------------------------------------------------------
cat > server/tests/fixtures.js << 'EOF'
